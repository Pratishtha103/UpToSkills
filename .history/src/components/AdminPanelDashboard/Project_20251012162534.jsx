      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading
          ? Array.from({ length: 6 }).map((_, idx) => (
              // 💡 CORRECTED: Added conditional dark mode classes for the skeleton loading card
              <div
                key={idx}
                className={`stat-card p-6 animate-pulse rounded-lg ${
                  isDarkMode
                    ? "bg-gray-800" // Dark background for dark mode skeleton
                    : "bg-white" // White background for light mode skeleton
                }`}
              />
            ))
