...
export default function Index() {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Failed to fetch students (status ${res.status})`);
        ...
        const formatted = rows.map((r) => {
          ...
          try {
            ...
            domains = r.domains_of_interest ? [String(r.domains_of_interest)] : [];
          }

          return {
            id: r.id ?? r.student_id ?? r._id ?? `student-${Math.random().toString(36).slice(2, 9)}`,
            full_name: r.full_name || r.name || r.student_name || "Unknown Student",
            domain: domains[0] || r.ai_skill_summary || "Web Development",
            skillLevel: r.skill_level || "Intermediate",
            badges: domains.slice(0, 4).length ? domains.slice(0, 4) : ["Profile"],
            ...
            created_at: r.created_at || r.profile_created_at || new Date().toISOString(),
  ...
  const handleViewProfile = (student) => {
    ...
    const s = student && (typeof student === "object" ? student : { id: student });
      ...
      Learn More
              ...
              <div className="hidden lg:block">
                <div style={{ width: 320, height: 200 }} className="bg-white/3 rounded-2xl flex items-center justify-center">
                  <div className="text-sm text-muted-foreground">3D Animation</div>
                </div>
              </div>

              <div className="mt-6 lg:mt-0">
                <motion.img src={boy} alt="Programmer" className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-[340px] h-auto mx-auto drop-shadow-2xl" style={{ borderRadius: "2rem" }} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.5, duration: 0.8 }} />
              </div>
            </div>
          </motion.section>
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 dark:text-foreground-white ">
              ...
              <StatCard title="Verified Badges" value="1,923" subtitle="Across all students" icon={Award} color="warning" delay={0.4} />
            </div>
          </section>

          {/* Student Section */}
          <section className="mb-8">
            <motion.h2 className="text-2xl font-bold text-foreground mb-6" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {loadingStudents ? (
                <div className="col-span-full text-center py-12">Loading students…</div>
              ) : students.length === 0 ? (
                <div className="col-span-full text-center py-12">No students found.</div>
              ) : (
                students.map((student, index) => (
                  <StudentCard key={student.id} student={student} onViewProfile={handleViewProfile} onContact={handleContact} delay={index * 0.06} />
                ))
              )}
            </div>
          </section>
