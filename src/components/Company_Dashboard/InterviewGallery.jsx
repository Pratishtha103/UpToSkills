...
export default function InterviewGallery() {
  ...
  useEffect(() => {
    const onCreated = (e) => {
      try {
        ...
        if (!d) return;
        const normalized = {
          id: d.id ?? d._id ?? d.interview_id ?? d.interviewId ?? null,
          candidate_name: d.candidate_name ?? d.candidateName ?? d.name ?? "",
          role: d.role ?? d.position ?? d.job ?? "",
          date: d.date ?? d.scheduled_date ?? d.slot_date ?? null,
          time: d.time ?? d.scheduled_time ?? d.slot_time ?? null,
          status: d.status ?? (d.state || "Scheduled"),
          raw: d.raw ?? d,
        };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      fetchInterviews();
      try { toast.success('Interview deleted'); } catch (e) { console.debug('toast error', e); }
    } catch (err) {
      console.error("Error deleting interview:", err);
      try { toast.error('Failed to delete interview'); } catch (e) { console.debug('toast error', e); }
    }
  };
