...
const Cybersecurity = () => {
    ...
    toastId = toast.info(
      <div className="w-full flex flex-col items-center gap-6 px-10 py-12 text-center">
        ...
        <div className="space-y-2 max-w-xl">
          <h3 className={`text-3xl font-semibold tracking-tight ${palette.headingClass}`}>
            Register Required
          </h3>
          <p className={`text-base leading-relaxed ${palette.bodyClass}`}>
            To apply for this program you need an UpToSkills account. Create one now so we can save your application
            progress and keep you posted about your status.
          </p>
        </div>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionResponse = await axios.post('http://localhost:5000/api/form', data, {
  ...
  return (
    ...
    >
      <div
        className={`absolute top-[-100px] left-[-100px] w-[300px] h-[300px] blur-3xl rounded-full animate-pulse ${
          darkMode ? 'bg-blue-600/20' : 'bg-blue-400/30'
        }`}
      />
      <div
        className={`absolute bottom-[-120px] right-[-100px] w-[320px] h-[320px] blur-3xl rounded-full animate-pulse ${
      ...
      <main className="flex-grow relative z-10 pb-20">
        <section className="text-center mt-28 mb-12 px-4">
          ...
          <p
            className={`max-w-3xl mx-auto text-lg leading-relaxed mb-3 ${
              darkMode ? 'text-gray-300' : 'text-gray-700'
            }`}
          >
            Learn to defend systems, networks, and data from cyber threats. Gain hands-on experience with ethical hacking,
            malware analysis, and digital forensics.
          </p>
          <p className="text-lg font-semibold text-indigo-500">Join us and master the art of cybersecurity defense.</p>
        </section>

        <div
          className={`max-w-4xl mx-auto backdrop-blur-md rounded-2xl p-6 shadow-lg mb-12 transform transition hover:scale-[1.01] ${
            darkMode ? 'bg-gray-800/60 border-gray-700' : 'bg-white/60 border-blue-100'
          } border`}
        >
          <p className={`leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
            <span className="font-bold text-blue-500">Skills:</span> Network Security • Ethical Hacking • Threat Analysis •
            Cryptography • Malware Detection • Wireshark & Kali Linux • Risk Management
          </p>
        </div>
        ...
        <div className="grid max-w-7xl mx-auto px-6 items-center">
          ...
          >
            <div
              className={`backdrop-blur-md p-10 rounded-3xl shadow-2xl transition-all duration-300 ${
                darkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-white/80 border-blue-100'
              } border hover:shadow-[0_10px_40px_rgba(59,130,246,0.2)]`}
            >
              ...
              <div className="space-y-6">
                ...
                <div className="relative">
                  ...
                  <label htmlFor="phonenum" className={labelClass}>
                    Phone Number
                  </label>
                </div>
                <div>
      ...
      <footer
        className={`w-full text-center py-4 text-sm transition-colors duration-300 border-t ${
          darkMode ? 'bg-gray-950 text-gray-300 border-gray-700' : 'bg-gray-700 text-gray-100 border-gray-300'
        }`}
      >
