...
import { 
    FaTrophy, FaCertificate, FaRegCheckCircle, FaClipboardCheck, 
    FaCode, FaUsers, FaLightbulb, FaUserGraduate 
} from 'react-icons/fa';

const AchievementCard = ({ id, name, description, isVerified, fullName, awardedAt }) => {

    // Convert comma-separated badges to array
    // Example: "HTML, CSS, React" → ["HTML", "CSS", "React"]
    const badgeList = name ? name.split(",").map(s => s.trim()) : [];

    // Style Mapping (based on first badge OR main award name)
    const primaryBadge = badgeList[0] || name;
    ...
    const styles = badgeStyles(primaryBadge);
    const AchievementIcon = styles.Icon;
    ...
    return (
        ...
        >
            ...
            {/* MULTIPLE BADGES DISPLAYED HERE */}
            <div className="flex flex-wrap justify-center gap-2 mb-3">
                {badgeList.map((badge, index) => (
                    <span
                        key={index}
                        className="px-3 py-1 text-sm bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-full border"
                    >
                        {badge}
                    </span>
                ))}
            ...
            <div className="mb-4">
                <span className={`inline-flex items-center px-4 py-1 text-sm font-bold rounded-full ${verificationBg} text-white`}>
